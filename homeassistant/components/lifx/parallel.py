    ...
    try:
        ...
        while command := pipe.recv():
            ...
            if command[0] == "QUERY_STATE":
                ...
                continue
            _kind, request_id, command_kind, payload = command
            packet_sequence = next_sequence()
            if command_kind == "color":
                ...
                packet = _set_waveform_optional(
                    source, packet_sequence, target, payload
                )
                ...
                pipe.send(("ERROR", request_id, f"unsupported command: {command_kind}"))
                continue
            if gc_was_enabled:
                gc.disable()
            pipe.send(("READY", request_id))
            dispatch_gate.acquire()
...
class LIFXParallelRuntime:
    ...
    def _start(self) -> None:
        ...
        self._dispatch_gate = context.Semaphore(0)
        ...
        try:
            for host in self.hosts:
                ...
                process = context.Process(
                    target=_worker,
                    args=(host, source, child, self._dispatch_gate, self._stop_event),
        ...
        with self._lock:
            ...
            try:
                for worker, command in zip(self._workers, commands, strict=True):
                    worker.pipe.send(
                        ("PREPARE", request_id, command.kind, command.payload)
                    )
